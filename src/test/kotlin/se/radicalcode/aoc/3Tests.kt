...
class TestFabric {
    @test
    fun testParseInput() {
        val fabricList = readFileAsStrings(ClassLoader.getSystemResource("3test-input.in").file)
    ...
    @test fun testFindOverlaps(){
        ...
        val fabricList2 = readFileAsStrings(ClassLoader.getSystemResource("3.in").file)
            .map{ FabricFactory.newFabricClaim(it)}
