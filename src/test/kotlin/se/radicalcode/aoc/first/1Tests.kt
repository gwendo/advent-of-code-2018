...
class TestFileRead {
    @test fun canReadFile() {
        val rows = readFile(ClassLoader.getSystemResource("testfile.in").file)
    ...
    @test fun solveTest() {
        assertEquals(3, reduceNumbers(readFile(ClassLoader.getSystemResource("testfile.in").file)))
    }

    @test fun solveOne() {
        println(reduceNumbers(readFile(ClassLoader.getSystemResource("1.in").file)))
    }

    @test fun solveStepTwo() {
        println(findDuplicateFrequencies(readFile(ClassLoader.getSystemResource("1.in").file)))
    }
}