...
class TestFileRead {
    @test fun canReadFile() {
        val rows = readFileAsInts(ClassLoader.getSystemResource("testfile.in").file)
    ...
    @test fun solveTest() {
        assertEquals(3,
            reduceNumbers(readFileAsInts(ClassLoader.getSystemResource("testfile.in").file))
        )
    }

    @test fun solveOne() {
        println(reduceNumbers(readFileAsInts(ClassLoader.getSystemResource("1.in").file)))
    }

    @test fun solveStepTwo() {
        println(findDuplicateFrequencies(readFileAsInts(ClassLoader.getSystemResource("1.in").file)))
    }
}